...
const Metadata = () => {
  ...
  const handleSaveConnection = async () => {
    ...
    try {
      if (isCreating) {
        ...
        updateConnection(selectedConnection.id, connectionData);
  ...
  const handleDeleteConnection = () => {
    if (selectedConnection && window.confirm('Вы уверены, что хотите удалить это подключение?')) {
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">
                  {isCreating ? 'Новое подключение' : 'Настройки подключения'}
                </h3>
                <div className="flex space-x-2">
                  {!isCreating && !isEditing && selectedConnection && (
                    <button
                      onClick={handleEdit}
              ...
              <div className="border-t pt-6">
                <button
                  ...
                  disabled={isTestingConnection || !selectedConnection}
